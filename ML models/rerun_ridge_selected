...
# Each horizon needs its own skip value for non-overlapping evaluation
HORIZONS = {
    'fwd_ret_10s': 1,
    'fwd_ret_30s': 3,
    'fwd_ret_1m':  6,
...
print(f"\n{'='*65}")
print(f"RIDGE BASELINE — ALL HORIZONS — SPY — 13 selected features")
print(f"{'='*65}")
...
for target, skip in HORIZONS.items():
    ...
    for train_df, test_df in folds:
        ic = run_fold(train_df, test_df, target, skip)
        fold_ics.append(ic)
...
out.to_csv(r"E:\quant-research-ofi\reports\SPY_ridge_all_horizons.csv", index=False)
print(f"\nSaved: SPY_ridge_all_horizons.csv")
